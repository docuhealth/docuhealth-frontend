...
interface UserSubAcctMedicalRecordsProps {
  subAcctMedicalRecords: MedicalRecord[];
  subAcctMedicalRecordsLoading: boolean;
  setViewDetailMedicalRecord: (value: boolean) => void;

  // Cleaned up props
  subAcctCount: number;
  subAcctCurrentPage: number;
  subAcctTotalPages: number;
  setSubAcctCurrentPage?: (page: number) => void; // Updated: use the setter instead of manual fetch
  fetchSubAcctMedicalRecords?: (page?: number) => void;

  setViewSubAcctDetailMedicalRecord: (value: boolean) => void;
  setSubAcctMedicalRecordsDetail: (record: MedicalRecord) => void;
}
...
}: UserSubAcctMedicalRecordsProps) => {
      ...
      <>
        <div className="py-14">
          ...
          <EmptyState
            ...
            description="No records yet, no hospital has dropped an after-visit summary."
          />
        </div>
      </>
    );
    ...
    <>
      <div className="bg-white my-5 border rounded-2xl pt-8 px-6 ">
        ...
        >
          ...
          <p className="text-docuhealth-dark">Go Back</p>
        </div>
        ...
        <div className="my-4 text-[12px] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {subAcctMedicalRecords.map((record) => (
            <div key={record.id} className="bg-docuhealth-bg-light border rounded-xl p-5">
        ...
        <Pagination2
          ...
          setCurrentPage={(page) => setSubAcctCurrentPage?.(page)}
        />
      </div>
    </>
  );
