...
const BookAppointment = ({ setBookAppointment, patientDetails }) => {
  ...
  const isToday =
    selectedDay === String(today.getDate()) &&
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axiosInstanceHos.get(
        `api/receptionists/staff/${selected === "Lab personnel" ? "lab_scientist" : selected.toLowerCase()}`,
      );
      ...
      setIsStaffSelectedRole(
        selected === "Lab personnel" ? "lab_scientist" : selected.toLowerCase(),
      );
  ...
  if (bookingSuccessful) {
    return (
      <AppointmentSuccessModal onClose={() => setBookAppointment(false)} />
    );
  }
  ...
  return (
    <Modal
      ...
      title={
        staffList.length > 0 && !isStaffSelected
          ? `Choose a preferred ${selected}`
          : "Appointment Request"
      }
            ...
            <div className="mb-2 relative ">
              ...
              >
                ...
                {isStaffSelectedRole === "nurse" && (
                  <option value="vital">
                    Vital checkup / other nursing services
                  </option>
                )}
            ...
            <div className="mb-4">
              <label className="flex items-center cursor-pointer group">
                <div className="relative">
                  ...
                  />
                  <div
                    className={`block w-8 h-4 rounded-full transition ${useTodayDate ? "bg-docuhealth-primary" : "bg-gray-300"} ${!isToday && !useTodayDate ? "opacity-50 cursor-not-allowed" : ""}`}
                  ></div>
                  <div
                    className={`dot absolute left-1 top-0.5 bg-white w-3 h-3 rounded-full transition-transform ${useTodayDate ? "translate-x-3" : ""}`}
                  ></div>
                </div>
                <div
                  className={`ml-2 font-medium text-[12px] ${!isToday && !useTodayDate ? "text-gray-400" : "text-gray-700"}`}
                >
                  Use today's date
                </div>
              </label>
