import { Pill, Building2, CalendarClock } from "lucide-react";
...
const getDrugsSummary = (order) => {
  if (order.drugs && order.drugs.length > 0) {
    const count = order.drugs.length;
...
const PrescriptionOrderCard = ({ order, badge, activeTab, onViewDetails }) => {
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col gap-4">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-[10px] text-slate-400 uppercase font-medium">Prescription</p>
            <p className="text-[13px] text-slate-600 truncate">{getDrugsSummary(order)}</p>
          </div>
          <div>
        ...
        {getRequestedBy(order) && (
          <div>
            <p className="text-[10px] text-slate-400 uppercase font-medium">Ordered By</p>
            <p className="text-[13px] text-slate-600 truncate">{getRequestedBy(order)}</p>
          </div>
        )}
      </div>
