...
const getDoctorName = (appt) => 
  appt.requested_by ||
  (appt.staff ? `Dr. ${appt.staff.firstname || ""} ${appt.staff.lastname || ""}`.trim() : "") ||
  "—";
...
const PharmacistAppointmentMobileCard = ({ appt, onSeeDetails, onCreateOrder, hideCreateOrder }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-md p-4">
      ...
      <div className="grid grid-cols-2 gap-4 pt-1">
        <div>
          <p className="text-[10px] text-gray-400 uppercase font-medium">Appointed Doctor</p>
          <p className="text-[13px] text-gray-600">{getDoctorName(appt)}</p>
        </div>
        <div>
...
export default PharmacistAppointmentMobileCard;
