      ...
      >
        <div className="p-4 flex justify-between items-center shrink-0">
          <div className="flex justify-start items-center gap-1 font-semibold text-docuhealth-primary">
            <img
              src={hospitalLogo || docuhealth_logo}
              alt="Logo"
            ...
            {navItems.map((item, index) => {
              ...
              return (
                <Link
                  ...
                  onClick={() => setOpenMobileSidebar(false)}
                >
                  <div className="px-4 my-4">{liContent}</div>
                </Link>
              );
            })}
          </ul>
            ...
            {bottomNavItems.map((item, index) => {
              ...
              return (
                <Link
                  ...
                  onClick={() => setOpenMobileSidebar(false)}
                >
                  <div className="px-4 my-2">{liContent}</div>
                </Link>
              );
            })}

            <div className="px-4 my-2 cursor-pointer" onClick={handleLogoutClick}>
              <li className="group px-4 py-2 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white rounded-lg flex items-center gap-2 justify-start transition-colors duration-150">
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
