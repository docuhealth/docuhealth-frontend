import React from "react";

const Features = () => {
  return (
    <div>
      <div className="flex flex-col justify-center items-center my-8" id="OurFeatures">
        <div className="bg-[#F6FDFF] py-3 px-5 rounded-full">
          <p className="text-sm">Our features</p>
        </div>
        <div className="">
          <h2 className="text-2xl font-semibold py-2 text-center">
            What's in your medical summary ?
          </h2>
        </div>
        <div>
          <div className="grid grid-cols-2 lg:grid-cols-4 grid-rows-3 mx-5 lg:mx-28 gap-5">
            <div className="shadow rounded-md p-4 col-span-2">
              <div className="py-2">
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clip-path="url(#clip0_2_169)">
                    <path
                      d="M7.07627 11.8638C6.85133 12.3789 6.13851 12.3789 5.9136 11.8638L5.3285 10.5237C4.80781 9.33115 3.87059 8.38182 2.70153 7.86288L1.09105 7.14802C0.579024 6.92075 0.579024 6.17587 1.09105 5.94859L2.65123 5.25604C3.85035 4.72376 4.80438 3.73929 5.31618 2.50603L5.90887 1.0779C6.12879 0.547927 6.86107 0.547925 7.081 1.0779L7.67367 2.50604C8.18547 3.73929 9.13947 4.72376 10.3386 5.25604L11.8988 5.94859C12.4108 6.17587 12.4108 6.92075 11.8988 7.14802L10.2883 7.86288C9.11927 8.38182 8.18207 9.33115 7.66133 10.5237L7.07627 11.8638ZM3.02637 6.54831C4.55027 7.22475 5.78935 8.33662 6.49492 9.86648C7.20053 8.33662 8.4396 7.22475 9.96347 6.54831C8.4214 5.8638 7.1784 4.69773 6.49493 3.14067C5.81146 4.69774 4.56845 5.8638 3.02637 6.54831ZM12.9343 15.1267L13.0988 14.7495C13.3921 14.0771 13.9205 13.5417 14.5797 13.2489L15.0866 13.0236C15.3608 12.9018 15.3608 12.5033 15.0866 12.3815L14.6081 12.1689C13.9319 11.8685 13.3941 11.3133 13.1057 10.618L12.9368 10.2105C12.819 9.92648 12.4263 9.92648 12.3085 10.2105L12.1396 10.618C11.8513 11.3133 11.3135 11.8685 10.6373 12.1689L10.1587 12.3815C9.8846 12.5033 9.8846 12.9018 10.1587 13.0236L10.6657 13.2489C11.3249 13.5417 11.8532 14.0771 12.1465 14.7495L12.3111 15.1267C12.4315 15.4027 12.8138 15.4027 12.9343 15.1267ZM12.2497 12.698L12.6247 12.3256L12.9919 12.698L12.6247 13.0599L12.2497 12.698Z"
                      fill="#70B0C3"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_2_169">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </div>
              <div>
                <h3 className="text-[#70B0C3] pb-1">Diagnosis History</h3>
                <p className="text-[#B9B9CA] text-sm">
                  Track your medical history with ease & access a clear record
                  of past conditions and treatments, View diagnoses, medications
                  in one place.
                </p>
              </div>
            </div>
            <div className="shadow rounded-md p-4 col-span-2 sm:col-span-1">
              <div className="py-2">
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clip-path="url(#clip0_2_179)">
                    <path
                      d="M7.07627 11.8638C6.85133 12.3789 6.13851 12.3789 5.9136 11.8638L5.3285 10.5237C4.80781 9.33115 3.87059 8.38182 2.70153 7.86288L1.09105 7.14802C0.579024 6.92075 0.579024 6.17587 1.09105 5.94859L2.65123 5.25604C3.85035 4.72376 4.80438 3.73929 5.31618 2.50603L5.90887 1.0779C6.12879 0.547927 6.86107 0.547925 7.081 1.0779L7.67367 2.50604C8.18547 3.73929 9.13947 4.72376 10.3386 5.25604L11.8988 5.94859C12.4108 6.17587 12.4108 6.92075 11.8988 7.14802L10.2883 7.86288C9.11927 8.38182 8.18207 9.33115 7.66133 10.5237L7.07627 11.8638ZM3.02637 6.54831C4.55027 7.22475 5.78935 8.33662 6.49492 9.86648C7.20053 8.33662 8.4396 7.22475 9.96347 6.54831C8.4214 5.8638 7.1784 4.69773 6.49493 3.14067C5.81146 4.69774 4.56845 5.8638 3.02637 6.54831ZM12.9343 15.1267L13.0988 14.7495C13.3921 14.0771 13.9205 13.5417 14.5797 13.2489L15.0866 13.0236C15.3608 12.9018 15.3608 12.5033 15.0866 12.3815L14.6081 12.1689C13.9319 11.8685 13.3941 11.3133 13.1057 10.618L12.9368 10.2105C12.819 9.92648 12.4263 9.92648 12.3085 10.2105L12.1396 10.618C11.8513 11.3133 11.3135 11.8685 10.6373 12.1689L10.1587 12.3815C9.8846 12.5033 9.8846 12.9018 10.1587 13.0236L10.6657 13.2489C11.3249 13.5417 11.8532 14.0771 12.1465 14.7495L12.3111 15.1267C12.4315 15.4027 12.8138 15.4027 12.9343 15.1267ZM12.2497 12.698L12.6247 12.3256L12.9919 12.698L12.6247 13.0599L12.2497 12.698Z"
                      fill="#AFB978"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_2_179">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </div>
              <div>
                <h3 className="text-[#AFB978] pb-1">Test Results</h3>
                <p className="text-[#B9B9CA] text-sm">
                  Get instant access to your lab results, uploaded directly by
                  hospitals.
                </p>
              </div>
            </div>
            <div className="shadow rounded-md p-4 col-span-2 sm:col-span-1">
              <div className="py-2">
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clip-path="url(#clip0_2_174)">
                    <path
                      d="M7.07627 11.8638C6.85133 12.3789 6.13851 12.3789 5.9136 11.8638L5.3285 10.5237C4.80781 9.33115 3.87059 8.38182 2.70153 7.86288L1.09105 7.14802C0.579024 6.92075 0.579024 6.17587 1.09105 5.94859L2.65123 5.25604C3.85035 4.72376 4.80438 3.73929 5.31618 2.50603L5.90887 1.0779C6.12879 0.547927 6.86107 0.547925 7.081 1.0779L7.67367 2.50604C8.18547 3.73929 9.13947 4.72376 10.3386 5.25604L11.8988 5.94859C12.4108 6.17587 12.4108 6.92075 11.8988 7.14802L10.2883 7.86288C9.11927 8.38182 8.18207 9.33115 7.66133 10.5237L7.07627 11.8638ZM3.02637 6.54831C4.55027 7.22475 5.78935 8.33662 6.49492 9.86648C7.20053 8.33662 8.4396 7.22475 9.96347 6.54831C8.4214 5.8638 7.1784 4.69773 6.49493 3.14067C5.81146 4.69774 4.56845 5.8638 3.02637 6.54831ZM12.9343 15.1267L13.0988 14.7495C13.3921 14.0771 13.9205 13.5417 14.5797 13.2489L15.0866 13.0236C15.3608 12.9018 15.3608 12.5033 15.0866 12.3815L14.6081 12.1689C13.9319 11.8685 13.3941 11.3133 13.1057 10.618L12.9368 10.2105C12.819 9.92648 12.4263 9.92648 12.3085 10.2105L12.1396 10.618C11.8513 11.3133 11.3135 11.8685 10.6373 12.1689L10.1587 12.3815C9.8846 12.5033 9.8846 12.9018 10.1587 13.0236L10.6657 13.2489C11.3249 13.5417 11.8532 14.0771 12.1465 14.7495L12.3111 15.1267C12.4315 15.4027 12.8138 15.4027 12.9343 15.1267ZM12.2497 12.698L12.6247 12.3256L12.9919 12.698L12.6247 13.0599L12.2497 12.698Z"
                      fill="#9E739B"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_2_174">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </div>
              <div>
                <h3 className="text-[#9E739B] pb-1">Vital signs tracking</h3>
                <p className="text-[#B9B9CA] text-sm">
                  Track your key health indicators, such as blood pressure.
                </p>
              </div>
            </div>
            <div className="shadow rounded-md p-4 col-span-2 sm:col-span-1">
              <div className="py-2">
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clip-path="url(#clip0_2_191)">
                    <path
                      d="M7.07627 11.8638C6.85133 12.3789 6.13851 12.3789 5.9136 11.8638L5.3285 10.5237C4.80781 9.33115 3.87059 8.38182 2.70153 7.86288L1.09105 7.14802C0.579024 6.92075 0.579024 6.17587 1.09105 5.94859L2.65123 5.25604C3.85035 4.72376 4.80438 3.73929 5.31618 2.50603L5.90887 1.0779C6.12879 0.547927 6.86107 0.547925 7.081 1.0779L7.67367 2.50604C8.18547 3.73929 9.13947 4.72376 10.3386 5.25604L11.8988 5.94859C12.4108 6.17587 12.4108 6.92075 11.8988 7.14802L10.2883 7.86288C9.11927 8.38182 8.18207 9.33115 7.66133 10.5237L7.07627 11.8638ZM3.02637 6.54831C4.55027 7.22475 5.78935 8.33662 6.49492 9.86648C7.20053 8.33662 8.4396 7.22475 9.96347 6.54831C8.4214 5.8638 7.1784 4.69773 6.49493 3.14067C5.81146 4.69774 4.56845 5.8638 3.02637 6.54831ZM12.9343 15.1267L13.0988 14.7495C13.3921 14.0771 13.9205 13.5417 14.5797 13.2489L15.0866 13.0236C15.3608 12.9018 15.3608 12.5033 15.0866 12.3815L14.6081 12.1689C13.9319 11.8685 13.3941 11.3133 13.1057 10.618L12.9368 10.2105C12.819 9.92648 12.4263 9.92648 12.3085 10.2105L12.1396 10.618C11.8513 11.3133 11.3135 11.8685 10.6373 12.1689L10.1587 12.3815C9.8846 12.5033 9.8846 12.9018 10.1587 13.0236L10.6657 13.2489C11.3249 13.5417 11.8532 14.0771 12.1465 14.7495L12.3111 15.1267C12.4315 15.4027 12.8138 15.4027 12.9343 15.1267ZM12.2497 12.698L12.6247 12.3256L12.9919 12.698L12.6247 13.0599L12.2497 12.698Z"
                      fill="#AFB978"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_2_191">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </div>
              <div>
                <h3 className="text-[#AFB978] pb-1">
                  No full clerking details
                </h3>
                <p className="text-[#B9B9CA] text-sm">
                  Hospitals can only upload summaries, not full internal
                  records.
                </p>
              </div>
            </div>
            <div className="shadow rounded-md p-4 col-span-2 sm:col-span-1">
              <div className="py-2">
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clip-path="url(#clip0_2_186)">
                    <path
                      d="M7.07627 11.8638C6.85133 12.3789 6.13851 12.3789 5.9136 11.8638L5.3285 10.5237C4.80781 9.33115 3.87059 8.38182 2.70153 7.86288L1.09105 7.14802C0.579024 6.92075 0.579024 6.17587 1.09105 5.94859L2.65123 5.25604C3.85035 4.72376 4.80438 3.73929 5.31618 2.50603L5.90887 1.0779C6.12879 0.547927 6.86107 0.547925 7.081 1.0779L7.67367 2.50604C8.18547 3.73929 9.13947 4.72376 10.3386 5.25604L11.8988 5.94859C12.4108 6.17587 12.4108 6.92075 11.8988 7.14802L10.2883 7.86288C9.11927 8.38182 8.18207 9.33115 7.66133 10.5237L7.07627 11.8638ZM3.02637 6.54831C4.55027 7.22475 5.78935 8.33662 6.49492 9.86648C7.20053 8.33662 8.4396 7.22475 9.96347 6.54831C8.4214 5.8638 7.1784 4.69773 6.49493 3.14067C5.81146 4.69774 4.56845 5.8638 3.02637 6.54831ZM12.9343 15.1267L13.0988 14.7495C13.3921 14.0771 13.9205 13.5417 14.5797 13.2489L15.0866 13.0236C15.3608 12.9018 15.3608 12.5033 15.0866 12.3815L14.6081 12.1689C13.9319 11.8685 13.3941 11.3133 13.1057 10.618L12.9368 10.2105C12.819 9.92648 12.4263 9.92648 12.3085 10.2105L12.1396 10.618C11.8513 11.3133 11.3135 11.8685 10.6373 12.1689L10.1587 12.3815C9.8846 12.5033 9.8846 12.9018 10.1587 13.0236L10.6657 13.2489C11.3249 13.5417 11.8532 14.0771 12.1465 14.7495L12.3111 15.1267C12.4315 15.4027 12.8138 15.4027 12.9343 15.1267ZM12.2497 12.698L12.6247 12.3256L12.9919 12.698L12.6247 13.0599L12.2497 12.698Z"
                      fill="#78B986"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_2_186">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </div>
              <div className="">
                <h3 className="text-[#78B986] pb-1">
                  No sensitive internal role
                </h3>
                <p className="text-[#B9B9CA] text-sm">
                  Protecting hospital documentation confidentiality.
                </p>
              </div>
            </div>
            <div className="shadow rounded-md p-4 col-span-2 row-span-2 relative">
              <div className="py-2">
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clip-path="url(#clip0_2_208)">
                    <path
                      d="M7.07627 11.8638C6.85133 12.3789 6.13851 12.3789 5.9136 11.8638L5.3285 10.5237C4.80781 9.33115 3.87059 8.38182 2.70153 7.86288L1.09105 7.14802C0.579024 6.92075 0.579024 6.17587 1.09105 5.94859L2.65123 5.25604C3.85035 4.72376 4.80438 3.73929 5.31618 2.50603L5.90887 1.0779C6.12879 0.547927 6.86107 0.547925 7.081 1.0779L7.67367 2.50604C8.18547 3.73929 9.13947 4.72376 10.3386 5.25604L11.8988 5.94859C12.4108 6.17587 12.4108 6.92075 11.8988 7.14802L10.2883 7.86288C9.11927 8.38182 8.18207 9.33115 7.66133 10.5237L7.07627 11.8638ZM3.02637 6.54831C4.55027 7.22475 5.78935 8.33662 6.49492 9.86648C7.20053 8.33662 8.4396 7.22475 9.96347 6.54831C8.4214 5.8638 7.1784 4.69773 6.49493 3.14067C5.81146 4.69774 4.56845 5.8638 3.02637 6.54831ZM12.9343 15.1267L13.0988 14.7495C13.3921 14.0771 13.9205 13.5417 14.5797 13.2489L15.0866 13.0236C15.3608 12.9018 15.3608 12.5033 15.0866 12.3815L14.6081 12.1689C13.9319 11.8685 13.3941 11.3133 13.1057 10.618L12.9368 10.2105C12.819 9.92648 12.4263 9.92648 12.3085 10.2105L12.1396 10.618C11.8513 11.3133 11.3135 11.8685 10.6373 12.1689L10.1587 12.3815C9.8846 12.5033 9.8846 12.9018 10.1587 13.0236L10.6657 13.2489C11.3249 13.5417 11.8532 14.0771 12.1465 14.7495L12.3111 15.1267C12.4315 15.4027 12.8138 15.4027 12.9343 15.1267ZM12.2497 12.698L12.6247 12.3256L12.9919 12.698L12.6247 13.0599L12.2497 12.698Z"
                      fill="#C3A670"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_2_208">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </div>
              <div>
                <h3 className="text-[#C3A670] pb-1">Treatment Summary</h3>
                <p className="text-[#B9B9CA] text-sm">
                  Stay on top of your medications, dosages, and schedules while
                  tracking procedures, surgeries, and hospitalizations with
                  ease, accessing detailed records of follow-up care and
                  appointments.
                </p>
              </div>

              <div class="hidden lg:flex gap-10 items-center my-20 justify-center text-[#8E8EA9]">
                <div class="bg-white text-gray-600 px-5 py-3 rounded text-center text-sm shadow w-56 transform rotate-45 ">
                  Track procedures easily
                </div>
                <div class="bg-white text-gray-600 px-5 py-3 rounded text-sm w-56 text-center shadow transform rotate-45">
                  Access detailed
                </div>
              </div>
            </div>
            <div className="shadow rounded-md p-4 col-span-2 sm:col-span-1">
              <div className="py-2">
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clip-path="url(#clip0_2_202)">
                    <path
                      d="M7.07627 11.8636C6.85133 12.3786 6.13851 12.3786 5.9136 11.8636L5.3285 10.5234C4.80781 9.3309 3.87059 8.38157 2.70153 7.86264L1.09105 7.14777C0.579024 6.9205 0.579024 6.17562 1.09105 5.94834L2.65123 5.2558C3.85035 4.72352 4.80438 3.73904 5.31618 2.50578L5.90887 1.07766C6.12879 0.547682 6.86107 0.547681 7.081 1.07766L7.67367 2.5058C8.18547 3.73904 9.13947 4.72352 10.3386 5.2558L11.8988 5.94834C12.4108 6.17562 12.4108 6.9205 11.8988 7.14777L10.2883 7.86264C9.11927 8.38157 8.18207 9.3309 7.66133 10.5234L7.07627 11.8636ZM3.02637 6.54806C4.55027 7.2245 5.78935 8.33637 6.49492 9.86624C7.20053 8.33637 8.4396 7.2245 9.96347 6.54806C8.4214 5.86355 7.1784 4.69748 6.49493 3.14042C5.81146 4.69749 4.56845 5.86355 3.02637 6.54806ZM12.9343 15.1264L13.0988 14.7493C13.3921 14.0769 13.9205 13.5415 14.5797 13.2486L15.0866 13.0234C15.3608 12.9016 15.3608 12.5031 15.0866 12.3813L14.6081 12.1686C13.9319 11.8682 13.3941 11.313 13.1057 10.6178L12.9368 10.2102C12.819 9.92624 12.4263 9.92624 12.3085 10.2102L12.1396 10.6178C11.8513 11.313 11.3135 11.8682 10.6373 12.1686L10.1587 12.3813C9.8846 12.5031 9.8846 12.9016 10.1587 13.0234L10.6657 13.2486C11.3249 13.5415 11.8532 14.0769 12.1465 14.7493L12.3111 15.1264C12.4315 15.4025 12.8138 15.4025 12.9343 15.1264ZM12.2497 12.6978L12.6247 12.3254L12.9919 12.6978L12.6247 13.0597L12.2497 12.6978Z"
                      fill="#9E739B"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_2_202">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </div>
              <div>
                <h3 className="text-[#9E739B] pb-1">Patient centered access</h3>
                <p className="text-[#B9B9CA] text-sm">
                  Only essential summaries for secure sharing.
                </p>
              </div>
            </div>{" "}
            <div className="shadow rounded-md p-4 col-span-2 sm:col-span-1">
              <div className="py-2">
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clip-path="url(#clip0_2_197)">
                    <path
                      d="M7.07627 11.8636C6.85133 12.3786 6.13851 12.3786 5.9136 11.8636L5.3285 10.5234C4.80781 9.3309 3.87059 8.38157 2.70153 7.86264L1.09105 7.14777C0.579024 6.9205 0.579024 6.17562 1.09105 5.94834L2.65123 5.2558C3.85035 4.72352 4.80438 3.73904 5.31618 2.50578L5.90887 1.07766C6.12879 0.547682 6.86107 0.547681 7.081 1.07766L7.67367 2.5058C8.18547 3.73904 9.13947 4.72352 10.3386 5.2558L11.8988 5.94834C12.4108 6.17562 12.4108 6.9205 11.8988 7.14777L10.2883 7.86264C9.11927 8.38157 8.18207 9.3309 7.66133 10.5234L7.07627 11.8636ZM3.02637 6.54806C4.55027 7.2245 5.78935 8.33637 6.49492 9.86624C7.20053 8.33637 8.4396 7.2245 9.96347 6.54806C8.4214 5.86355 7.1784 4.69748 6.49493 3.14042C5.81146 4.69749 4.56845 5.86355 3.02637 6.54806ZM12.9343 15.1264L13.0988 14.7493C13.3921 14.0769 13.9205 13.5415 14.5797 13.2486L15.0866 13.0234C15.3608 12.9016 15.3608 12.5031 15.0866 12.3813L14.6081 12.1686C13.9319 11.8682 13.3941 11.313 13.1057 10.6178L12.9368 10.2102C12.819 9.92624 12.4263 9.92624 12.3085 10.2102L12.1396 10.6178C11.8513 11.313 11.3135 11.8682 10.6373 12.1686L10.1587 12.3813C9.8846 12.5031 9.8846 12.9016 10.1587 13.0234L10.6657 13.2486C11.3249 13.5415 11.8532 14.0769 12.1465 14.7493L12.3111 15.1264C12.4315 15.4025 12.8138 15.4025 12.9343 15.1264ZM12.2497 12.6978L12.6247 12.3254L12.9919 12.6978L12.6247 13.0597L12.2497 12.6978Z"
                      fill="#70B0C3"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_2_197">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </div>
              <div>
                <h3 className="text-[#70B0C3] pb-1">Top security</h3>
                <p className="text-[#B9B9CA] text-sm">
                  End-to-end encryption for total and complete security.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Features;
