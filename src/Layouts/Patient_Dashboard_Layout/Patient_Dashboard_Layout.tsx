...
const Patient_Dashboard_Layout = () => {
  ...
  const customBottomContent = (
    <div className="shrink-0 px-4 pt-2">
      <div className="flex justify-between items-center py-2 text-gray-700 cursor-pointer text-sm">
        <p>Emergency Mode</p>
  ...
  return (
    <>
      <DashboardLayout
        profile={profile}
        hospitalLogo={docuhealth_logo}
        hospitalName="DocuHealth"
        roleLabel={profile?.email || "Patient"}
        navItems={navItems}
      ...
      {emergencyStatusModal && (
        <EmergencyModeNotice
          emergencyStatusModal={emergencyStatusModal}
          setEmergencyStatusModal={setEmergencyStatusModal}
          handleEmergencyStatusModal={handleEmergencyStatusModal}
        />
      )}
    </>
  );
