...
const FAQ_Section = () => {
  ...
  const toggleAccordion = (index) => {
    ...
    return (
        <>
            <BackgroundTemplate>
                <section className="py-24 px-16 text-sm text-[#464646]">
      ...
      <div className="w-full md:w-1/2">
        ...
        <div className="space-y-3">
          {faqData.map((item, index) => (
            <div
              key={index}
              className={`bg-[#f5f9ff] ${openIndex === index ? 'rounded-sm' : 'rounded-full'} overflow-hidden shadow-sm transition-all duration-100`}
            >
              <button
              ...
              >
                ...
                <Plus
                  className={`w-5 h-5 text-[#3E4095] transform transition-transform duration-500 ${
                    openIndex === index ? "rotate-[360deg]" : "rotate-0"
                  }`}
                />
              </button>

              <div
                className={`px-5 text-sm text-gray-600 transition-all duration-500 ease-in-out ${
                  openIndex === index
